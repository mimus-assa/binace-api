    ...
    if request.method == 'POST':
        try:
            ...
            while True:
                ...
                if side == SIDE_BUY:
                    if current_price <= price_a:
                        result = client.create_margin_order(symbol="BTCBUSD", side=side, type="LIMIT", quantity=quantity, price=price_a, sideEffectType="MARGIN_BUY", isIsolated=True)
                        break
                elif side == SIDE_SELL:
                    if current_price >= price_a:
                        result = client.create_margin_order(symbol="BTCBUSD", side=side, type="LIMIT", quantity=quantity, price=price_a, sideEffectType="MARGIN_SELL", isIsolated=True)
                        break
